  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      if (Config.enableAWSVerifiedPermissions) {
        ...
        let userEntities: AWS.VerifiedPermissions.EntitiesDefinition = getUserEntitiesFromToken(actionPayload); 
        ...
        if (userId) {
          ...
          const isAuthorizedForAction = await isAuthorized(
            { entityType: 'User', entityId: userId }, resource, {
            actionType: 'Action',
            actionId: actionType,
          }, recommendationResponse?.risk_score, userEntities);
